...
public class ReviewActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    public void review(String review){
        ...
        if (DBUtils.isView ){
            Toast.makeText(this, "现在是查看不是复查", Toast.LENGTH_SHORT).show();
        }else {
            progressDialog = ProgressDialog.show(this, "提示",
                    "提交中……", false, true);
            final Danger danger;
            final JSONObject jsonObject = JSON.parseObject(review);
            System.out.println("jsonObject = " + jsonObject);
            danger = jsonObject.toJavaObject(Danger.class);
            System.out.println("danger = " + danger.getDangerState());
            Thread thread = new Thread(new Runnable() {
                @Override
                public void run() {
                    int dangerId = DBUtils.review(danger);
    ...
    public void initView(){
        ...
        webView.loadUrl("file:///android_asset/views/review.html");//加载web页面
    ...
    public void initData() {
        ...
        webView.loadUrl("javascript:initData('"
                + dangerAreas + "','" + dangerTypes + "','"
                + companys + "'," + DBUtils.user.getId() + ",'"+
                DBUtils.api+"','"+DBUtils.imgUrl+"')");
        if (DBUtils.isReview){
            webView.loadUrl("javascript:setDangerData('"+DBUtils.dangerJson+"',"+0+")");
        }
        if (DBUtils.isView){
            webView.loadUrl("javascript:setDangerData('"+DBUtils.dangerJson+"',"+1+")");
        }
    }
        ...
        Thread thread = new Thread(new Runnable() {
            ...
            public void run() {
                String dangersJson = DBUtils.queryDangers(
                        "/queryDanger?dangerState=2&createPId="+
                                DBUtils.user.getId() );
                Message msg = new Message();
                msg.what = 1002;
                Bundle data = new Bundle();
    ...
    public void uploadImg(final String imgdata, final int dangerId){
        final Map<String,String> map = new HashMap<>();
        map.put("base64",imgdata);
        map.put("dangerId",dangerId+"");
        map.put("flag","2");
        Thread thread = new Thread(new Runnable() {
            @Override
            public void run() {
                String rs = httpUtil.PostMethod(
                        DBUtils.api+"/uploadImg",map,"UTF-8");
                Message msg = new Message();
                msg.what = 1001;
                Bundle data = new Bundle();
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if ((keyCode == KeyEvent.KEYCODE_BACK)) {
                DBUtils.isView = false;
                DBUtils.isReview = false;
                super.onPause();
                super.onStop();
                super.finish();
            return false;
        ...
        public void handleMessage(android.os.Message msg) {
            switch (msg.what) {
                ...
                case 1004:
                    ...
                    if (dangerId != 0)
                    {
                        Toast.makeText(getApplicationContext(), "提交复查成功",
                                Toast.LENGTH_SHORT).show();
