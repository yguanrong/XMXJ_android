...
/**
 * Created by GuanRong on 2019/4/3.
 */
...
public class AddRecordActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    @JavascriptInterface
    public void addNewDanger(String newDanger){

        JSONObject jsonObject = JSON.parseObject(newDanger);
        final Danger danger = jsonObject.toJavaObject(Danger.class);
        progressDialog = ProgressDialog.show(this, "提示",
                "提交中……", false, true);
        if (DBUtils.isEdit){
            System.out.println(" 编辑danger");
            Thread thread = new Thread(new Runnable() {
                @Override
                public void run() {
                    int dangerId = DBUtils.updateDanger(danger);
            ...
            System.out.println(" 新增danger");
            Thread thread = new Thread(new Runnable() {
                @Override
                public void run() {
                    int dangerId = DBUtils.newDanger(danger);
                    Message msg =new Message();
                    msg.what = 1004;
                    Bundle data = new Bundle();
                    data.putInt("result",dangerId);
    ...
    public void initView(){
        ...
        webView.loadUrl("file:///android_asset/views/newRecord.html");//加载web页面
        ...
        webView.setWebViewClient(new WebViewClient() {
            ...
            public void onPageFinished(WebView view, String url) {
                ...
                initData();
                webView.loadUrl("javascript:setCreateUserId("+
                        DBUtils.user.getId() +")");
    ...
    public void initData(){
        ...
        Thread thread =new Thread(new Runnable() {
            @Override
            public void run() {
                List<Data> dangerAreas = DBUtils.queryDangerArea();
                List<Data> dangerTypes = DBUtils.queryDangerType();
                List<Data> company = DBUtils.queryCompanyPart(2);
                List<Data> items = DBUtils.queryItem(DBUtils.user.getId());

                Message msg = new Message();
                msg.what = 1002;
                Bundle data = new Bundle();
                data.putString("dangerAreas", JSON.toJSONString(dangerAreas));
                data.putString("dangerTypes", JSON.toJSONString(dangerTypes));
                data.putString("company", JSON.toJSONString(company));
    ...
    //根据公司id查找用户，
    @JavascriptInterface
    public void queryUserByCompany(final int companyId){
        Thread thread =new Thread(new Runnable() {
            @Override
            public void run() {
                List<Data> company = DBUtils.queryUserByCompany(companyId,3);

                Message msg = new Message();
                msg.what = 1003;
                Bundle data = new Bundle();
                data.putString("userBycompanyId", JSON.toJSONString(company));
                msg.setData(data);
                mHandler.sendMessage(msg);
            }
        });
        ...
        public void handleMessage(android.os.Message msg) {
            switch (msg.what) {
                case 1001:
                    ...
                    break;
                case 1002:
                    String dangerArea = msg.getData().getString("dangerAreas");
                    String dangerTypes = msg.getData().getString("dangerTypes");
                    String company = msg.getData().getString("company");
                    String items = msg.getData().getString("items");

                    webView.loadUrl("javascript:initData('"+dangerArea+"','"+
                            dangerTypes+"','"+company+"','"+items+"')");
                    if (DBUtils.isEdit){
                        System.out.println("DBUtils.dangerJson = " + DBUtils.dangerJson);
                        webView.loadUrl("javascript:setEditData('"+ DBUtils.dangerJson +"')");
                    }
                    break;
                case 1003:
                    String userBycompanyId = msg.getData().getString("userBycompanyId");
                    System.out.println("userBycompanyId = " + userBycompanyId);
                    webView.loadUrl("javascript:initUser('"+userBycompanyId+"')");
                    break;
                case 1004:
                    dangerId = msg.getData().getInt("result");
                    if (dangerId != 0)
                    {
                        Toast.makeText(getApplicationContext(), "新增记录成功",
                                Toast.LENGTH_SHORT).show();

                        if (imgdata!=null && !imgdata.equals(""))
                        {
                            uploadImg(imgdata,dangerId,1);
                        }
                    }
                    else {
                        Toast.makeText(getApplicationContext(), "新增记录失败",
                                Toast.LENGTH_SHORT).show();
