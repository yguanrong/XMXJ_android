...
public class DangerAreaActivity extends AppCompatActivity {
    ...
    public void initView(){
        ...
        webView.loadUrl("file:///android_asset/views/dangerArea.html");//加载web页面
        ...
        switch (DBUtils.user.getIdentify()){
            case 0:
                if (DBUtils.itemOrCompany == 1){
                    webView.loadUrl("javascript:setItem('"+
                            JSON.toJSONString(DBUtils.item)+"','"+DBUtils.api+"')");

                }else {
                    Toast.makeText(this, "暂无分包公司区域分析", Toast.LENGTH_SHORT).show();
                }
                break;
            case 1:
                webView.loadUrl("javascript:setItem('"+
...
//        webView.loadUrl("javascript:setItem('"+ JSON.toJSONString(DBUtils.item)+"')");
    }
}
